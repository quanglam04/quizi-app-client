...
const ClassroomPage = () => {
  ...
  const [expandedClassroom, setExpandedClassroom] = useState<string | null>(
    null,
  );
  ...
  const { data: leaderboard, isLoading: isLoadingLeaderboard } = useQuery<
    LeaderboardEntry[]
  >({
    queryKey: ["classroom-leaderboard", expandedClassroom],
    queryFn: async () => {
      const res = await api.get(`/classrooms/${expandedClassroom}/leaderboard`);
      return res.data;
    },
    enabled: !!expandedClassroom,
  });
    ...
    <div className="space-y-12">
      ...
      <div>
        ...
        <p className="text-white/40 mt-1">
          Tham gia và quản lý các lớp học của bạn
        </p>
      </div>
        ...
        {isLoading ? (
          <div className="grid gap-6 sm:grid-cols-2">
            {[1, 2].map((i) => (
              <div
                key={i}
                className="h-40 bg-slate-800/50 rounded-2xl border border-white/10 animate-pulse"
              />
            ))}
          </div>
        ) : classrooms?.length === 0 ? (
          ...
          <div className="grid gap-6 sm:grid-cols-2">
            {classrooms?.map((cls) => (
              <React.Fragment key={cls.classroomId}>
                <div
                ...
                >
                  <div className="flex justify-between items-start mb-4">
                    <div className="min-w-0">
                      <h3
                        className="text-xl font-bold text-white truncate group-hover:text-sky-400 transition-colors"
                        title={cls.name}
                      >
                        {cls.name}
                      </h3>
                  ...
                  <div className="flex justify-between items-center pt-4 border-t border-white/5">
                    {/* Nút xem bảng xếp hạng — chỉ hiện khi đã được duyệt */}
                    {cls.status === "approved" ? (
                      <button
                        onClick={() =>
                        ...
                        className="text-sky-400/60 hover:text-sky-400 text-xs font-bold uppercase tracking-widest transition-colors flex items-center gap-1.5"
                      >
                        🏆{" "}
                        {expandedClassroom === cls.classroomId
                          ? "Ẩn xếp hạng"
                ...
                {expandedClassroom === cls.classroomId && (
                  <div className="bg-slate-900 rounded-2xl border border-sky-500/20 overflow-hidden mb-6 sm:col-span-2">
                    <div className="px-6 py-4 border-b border-white/5 flex items-center justify-between">
                      <h3 className="text-sm font-bold text-white">
                        🏆 Bảng xếp hạng — {cls.name}
                      </h3>
                      <p className="text-white/30 text-xs">
                        Điểm cao nhất trung bình tất cả đề
                      </p>
                    </div>

                    {isLoadingLeaderboard ? (
                      <div className="py-10 text-center">
                        <div className="animate-spin h-6 w-6 border-4 border-sky-500 border-t-transparent rounded-full mx-auto" />
                      </div>
                    ) : (
                      <table className="min-w-full divide-y divide-white/5">
                        <thead className="bg-slate-800/50">
                          <tr>
                            <th className="px-4 py-3 text-left text-xs font-bold text-white/30 uppercase tracking-widest w-12">
                              Hạng
                            </th>
                            <th className="px-4 py-3 text-left text-xs font-bold text-white/30 uppercase tracking-widest">
                              Học sinh
                            </th>
                            <th className="px-4 py-3 text-center text-xs font-bold text-white/30 uppercase tracking-widest">
                              Số đề đã làm
                            </th>
                            <th className="px-4 py-3 text-right text-xs font-bold text-white/30 uppercase tracking-widest">
                              Điểm TB
                            </th>
                          </tr>
                        </thead>
                        <tbody className="divide-y divide-white/5">
                          {leaderboard?.length === 0 ? (
                            <tr>
                              <td
                                colSpan={3}
                                className="px-4 py-8 text-center text-white/20 italic text-sm"
                              >
                            ...
                            leaderboard?.map((entry) => (
                              ...
                              >
                                ...
                                <td className="px-4 py-3 whitespace-nowrap text-center text-sm text-white/40">
                                  {entry.examCount > 0 ? (
                                    `${entry.examCount} đề`
                                  ) : (
